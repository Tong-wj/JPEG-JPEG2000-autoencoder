...
# from cae_32x32x32_mirror_pad_round import CAE
# from cae_96x16x16 import CAE
...
def train(cfg: Namespace) -> None:
    ...
    # train-related code
    model = CAE()
    # model.load_state_dict(T.load(cfg.chkpt))
    model.train()
    if cfg.device == "cuda":
        model.cuda()
    ...
    for epoch_idx in range(cfg.start_epoch, cfg.num_epochs + 1):
        ...
        for batch_idx, data in enumerate(dataloader, start=1):
            ...
            # if (avg_loss < 0.01) and (avg_loss >= 0.005) and (cfg.learning_rate != 7e-5):
            #     cfg.learning_rate = 7e-5
            #     logger.debug('learning_rate:7e-5')
            # if (avg_loss < 0.005) and (avg_loss >= 0.002) and (cfg.learning_rate != 3e-5):
            #     cfg.learning_rate = 3e-5
            #     logger.debug('learning_rate:3e-5')
            # if (avg_loss < 0.002) and (cfg.learning_rate != 1e-5):
            #     cfg.learning_rate = 1e-5
            #     logger.debug('learning_rate:1e-5')
            ...
            if batch_idx % cfg.save_every == 0:
                ...
                save_imgs(imgs=y, to_size=(3, 128, 2 * 128), name=f"../experiments/{cfg.exp_name}/out/out_{epoch_idx}_{batch_idx}.png")
        ...
        if epoch_idx % cfg.epoch_every == 0:
            ...
            writer.add_scalar("train/epoch_avg_loss", epoch_avg / cfg.epoch_every, epoch_idx // cfg.epoch_every)

            logger.info("Epoch avg = %.8f" % epoch_avg)
            epoch_avg = 0.0
            T.save(model.state_dict(), f"../experiments/{cfg.exp_name}/chkpt/model_96x16x16_round_{epoch_idx}.pth")
